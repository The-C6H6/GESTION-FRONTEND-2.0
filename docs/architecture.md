...
Infrastructure adapters
    ├── HTTP: login, enrolled-student, failed-subject, ruling-create/search/update/delete APIs
...
La composición ocurre en `core/services.py`. `build_services()` comparte un `ApiClient` y un `AuthTokenStore` entre `ApiAuthRepository`, `ApiInscritoRepository`, `ApiReprobadoRepository` y `ApiDictamenRepository`. El controlador conserva `DemoAlumnoRepository` y `DemoDictamenRepository` para compatibilidad, pero recibe por separado los repositorios HTTP enfocados de reprobados, creación, búsqueda, modificación y eliminación. `build_demo_services()` continúa disponible para pruebas aisladas.
