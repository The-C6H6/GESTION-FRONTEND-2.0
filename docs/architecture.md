...
La aplicación es un frontend Flet web con navegación declarativa y sesión en memoria. La autenticación, la consulta de alumnos inscritos, la consulta de materias reprobadas, y la creación, búsqueda paginada, modificación y eliminación de dictámenes usan el backend real. El registro de usuarios y la generación PDF permanecen en modo demostración; no existe autenticación demo ni fallback de identidad. Los siguientes adaptadores HTTP podrán sustituir a los demo sin cambiar las vistas.
...
Infrastructure adapters
    ├── HTTP: login/identity/refresh, enrolled-student, failed-subject, ruling-create/search/update/delete APIs
    └── Demo: user registration, PDF, and compatibility repositories
```
...
La composición ocurre en `core/services.py`. `build_services()` comparte un `ApiClient` y un único `AuthSessionStore` entre `ApiAuthRepository`, los adaptadores HTTP y los guards de los controladores. El controlador conserva `DemoAlumnoRepository` y `DemoDictamenRepository` para compatibilidad, pero recibe por separado los repositorios HTTP enfocados de inscritos, reprobados, creación, búsqueda, modificación y eliminación. Las pruebas construyen sus propias sesiones y servicios no conectados; producción no ofrece login ni sesión demo.

El registro real de usuarios y la generación de PDF continúan fuera del alcance actual. El adaptador PDF futuro deberá basarse en las referencias de `referencias/` y consumir el `PdfRequest` ya separado del payload HTTP.
