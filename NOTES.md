...
- HTTP adapters for registration, rulings, and the real PDF remain intentionally unimplemented. `/api/auth/me` and `/api/auth/refresh` are also deferred. Creating rulings through `POST /api/dictaminaciones` is the recommended next API integration.
